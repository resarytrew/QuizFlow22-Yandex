  ...
  return (
    <BaseNode title="Переменная" icon={Icon} nodeProps={props} color="orange">
      <div className="font-mono text-xs space-y-1.5 bg-gray-50/80 p-2 rounded-md border border-gray-200/75">
        ...
        <div className="truncate"><span className="text-gray-500">TO</span> <span className="text-orange-700 font-semibold" title={String(data.value)}>{`"${String(data.value)}"` || '...'}</span></div>
      </div>
    </BaseNode>
  );
};
